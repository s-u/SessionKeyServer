...
public class SessionKeyServer {
    ...
    public static void main(String[] args) throws IOException, KeyStoreException {
        ...
        while (i < args.length) {
            ...
            else if (args[i].equals("-krb5conf") && ++i < args.length) krb5conf = args[i];
        ...
        try {
            ...
            if (requestMethod.equalsIgnoreCase("GET")) {
                if (requestPath.equals("/valid")) {
                    ...
                    String module = "PAM";
                    boolean succ = false;
                    if (com.att.research.RCloud.JaasAuth.checkUser(user, pwd, module, SessionKeyServer.krb5conf)) {
                        md.update(java.util.UUID.randomUUID().toString().getBytes());
                        md.update(java.util.UUID.randomUUID().toString().getBytes());
                        String sha1 = bytes2hex(md.digest());
                ...
                } else if (requestPath.equals("/authorize_token")) {
                    String user = queryMap.get("user");
                    char[] pwd = queryMap.get("pwd").toCharArray();
                    String moduledefault = "PAM";
                    ...
                    System.out.println(module + ": " + ((new Date()).getTime()) + " user='" + user + "', " + realm_txt + ", " + (succ ? "OK" : "FAILED"));
                } else {
